...
class AddableFoodItem extends Component {
	...
	handleSizeChange(newServingSizeId) {
		let newServingSize = this.state.item.servingSizes.find(
	...
	handleItemClick() {
		if (!this.props.editMode) {
			...
			if (!this.state.adding && !this.state.item) {
				...
				fetch(
					deploymentConfig().apiUrl + "/api/foods/" + this.props.item.foodItemId
				)
					.then(resp => resp.json())
					.then(item => {
	...
	setDefaultServingQuantity(item) {
		if (
			item.selectedServing.servingSize.label === "g" &&
			item.selectedServing.quantity === 1
		) {
	...
	calculateItemTotals() {
		...
		let servingSizeMultiplier =
			item.selectedServing.quantity * item.selectedServing.servingSize.ratio;
		...
		fetch(
			...
			if (res.ok) {
				window.location.hash =
					deploymentConfig().baseUrl + "#/?day=" + this.props.day;
	...
	addFoodToMeal() {
		...
		fetch(
			deploymentConfig().apiUrl +
				...
				this.props.mealGroupContext.id +
				"/add?token=" +
				localStorage.getItem("token"),
			{
				method: "POST",
			...
			if (!res.ok) {
				alert(
					"There was an error adding this food to " +
						this.props.mealGroupContext.name
				);
			} else {
				...
				{this.state.item && (
					<span>
						<span className="MealItem__macros">
							<span className="MealItem__macros--carbs">
								{itemTotals.carbs}
							</span>
							<span className="MealItem__macros--fat">{itemTotals.fat}</span>
							<span className="MealItem__macros--protein">
								{itemTotals.protein}
							</span>
						</span>
