...
class MyMeals extends Component {
	...
	handleDefaultQuantityChange(mealId, mealGroupItemId, newQuantity) {
		const { mealIdx, mealGroupItemIdx } = this.findMealAndGroupItem(
			mealId,
			mealGroupItemId
		);
	...
	handleDefaultServingChange(mealId, mealGroupItemId, newServingSizeId) {
		const {
			...
			mealGroupItem,
			mealGroupItemIdx
		} = this.findMealAndGroupItem(mealId, mealGroupItemId);
		const newDefaultServingSize = mealGroupItem.foodItem.servingSizes.find(
			servingSize => servingSize.id === newServingSizeId
		);
	...
	updateMeal(mealId) {
		...
		fetch(
			deploymentConfig().apiUrl +
				"/api/food-meal-groups/update?token=" +
				localStorage.getItem("token"),
			{
				method: "POST",
	...
	findMealAndGroupItem(mealId, mealGroupItemId) {
		...
		const mealGroupItem = meal.mealGroupItems.find(
			groupItem => groupItem.id === mealGroupItemId
		);
	...
	handleNewMealItemNameChange(itemName) {
		this.setState(
			...
			() => {
				this.updateMealItemName(
					this.state.mealItemToCreate.id,
					this.state.mealItemToCreate.name
				);
			}
		);
	}
			...
			<div className={"MyMeals" + (this.props.editMode ? " edit-mode" : "")}>
				{this.state.meals.map(meal => (
					<AddableMealItem
						...
						handleDefaultQuantityChange={this.handleDefaultQuantityChange.bind(
							this
						)}
						handleDefaultServingChange={this.handleDefaultServingChange.bind(
							this
						)}
						handleMealItemRemoved={this.handleMealItemRemoved.bind(this)}
						handleExistingMealItemNameChange={this.handleExistingMealItemNameChange.bind(
							this
						)}
				...
				<CreateMealItem
					...
					handleMealItemRemoved={this.handleMealItemRemoved.bind(this)}
					handleNewMealItemNameChange={this.handleNewMealItemNameChange.bind(
						this
					)}
					updateMeal={this.updateMeal.bind(this)}
					onCreateMealItemClicked={this.handleCreateMealItemClicked.bind(this)}
					deleteNewlyCreatedMealItem={this.deleteNewlyCreatedMealItem.bind(
						this
					)}
				/>
			</div>
